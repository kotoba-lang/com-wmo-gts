(ns wmo_gts.main
  "Kotodama WASM entrypoint for the Wmo_gts clean-room actor (L4) — Clojure port.
  ...
  facts (`wmo_gts.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "wmo_gts")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"wmo_gts.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wmo_gts-compat" :tier tier :entities entities} 200])
...
(defn start! [] :wmo_gts-compat/ready)
